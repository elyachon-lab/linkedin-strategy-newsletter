...
export function AuditDiagnosticSkeleton({ queryName = 'le profil' }: { queryName?: string }) {
  return (
    <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 brand-card-shadow space-y-6 animate-pulse">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 text-indigo-900 text-xs font-extrabold border border-indigo-200">
            <Activity className="w-3.5 h-3.5 text-indigo-600 animate-spin" /> PHASE 1 : SCAN & EXTRACTION DES DONNÉES EN COURS...
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-indigo-950 pt-1 flex items-center gap-2">
            <span>Scan sémantique de {queryName}...</span>
          </h2>
          <div className="h-3 bg-slate-200 rounded-full w-3/4 max-w-md"></div>
        </div>

        <div className="bg-indigo-50 text-indigo-950 px-6 py-4 rounded-2xl border border-indigo-200 text-center shrink-0 space-y-1">
          <div className="h-3 bg-indigo-200 rounded w-20 mx-auto"></div>
          <div className="h-8 bg-indigo-300 rounded w-16 mx-auto animate-pulse"></div>
        </div>
      </div>
      ...
      <div className="p-4 bg-indigo-50/60 border border-indigo-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Sparkles className="w-6 h-6 text-indigo-600 shrink-0 animate-bounce" />
          <div className="space-y-1.5 flex-1">
            <div className="h-4 bg-indigo-200 rounded w-64"></div>
            <div className="h-3 bg-indigo-100 rounded w-80"></div>
          </div>
        </div>
        <div className="h-8 bg-indigo-200 rounded-xl w-32 shrink-0"></div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-3">
          <h3 className="text-xs font-extrabold uppercase text-slate-700 flex items-center gap-2">
            <PieChart className="w-4 h-4 text-indigo-600" /> Extractions des formats de publication...
          </h3>
