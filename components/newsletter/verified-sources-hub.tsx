...
export function VerifiedSourcesHub({ onImportSource }: VerifiedSourcesHubProps) {
  return (
    <div className="bg-white p-6 rounded-3xl border border-indigo-100 brand-card-shadow space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {VERIFIED_PLATFORM_NEWS.map((item, idx) => (
          <div
            key={idx}
            className="p-4 rounded-2xl border border-slate-200 bg-slate-50/80 space-y-2 hover:border-indigo-600 transition-all"
          >
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-extrabold text-cyan-300 bg-indigo-950 px-2.5 py-0.5 rounded-full border border-indigo-800">
                {item.platform}
              </span>
            ...
            <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-[11px]">
              ...
              {onImportSource && (
                <button
                  ...
                  className="px-2.5 py-1 bg-indigo-950 text-cyan-300 font-extrabold rounded-lg hover:bg-slate-900 transition-colors shrink-0 ml-2"
                >
                  + Importer
                </button>
              )}
